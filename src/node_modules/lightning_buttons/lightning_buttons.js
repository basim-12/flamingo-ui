...
async function lightning_buttons(opts = {}, protocol) {
  ...
  // -------------------- Protocol --------------------
  let sendUp = () => { }
  let currentLnBalance = 0
  let currentTemplates = []
  let currentContacts = []

  if (protocol) {
    sendUp = protocol(msg => {
      console.log('[lightning_buttons]:', msg)
      if (msg.type === 'ln_invoice_decoded' && pay_el && typeof pay_el.onDecoded === 'function') {
        pay_el.onDecoded(msg.data)
      }
      if (msg.type === 'ln_invoice_paid' && pay_el && typeof pay_el.onPaid === 'function') {
        pay_el.onPaid(msg.data)
      }
      if (msg.type === 'ln_invoice_created' && create_el && typeof create_el.onCreated === 'function') {
        create_el.onCreated(msg.data)
      }
      if (msg.type === 'ln_balance_updated') {
        currentLnBalance = msg.data?.balance || 0
        if (create_el && typeof create_el.updateBalance === 'function') {
          create_el.updateBalance(currentLnBalance)
        }
      }
      if (msg.type === 'ln_templates_updated') {
        currentTemplates = msg.data?.templates || []
        if (create_el && typeof create_el.updateTemplates === 'function') {
          create_el.updateTemplates(currentTemplates)
        }
      }
      if (msg.type === 'contacts_updated') {
        currentContacts = msg.data?.contacts || []
        if (create_el && typeof create_el.updateContacts === 'function') {
          create_el.updateContacts(currentContacts)
        }
      }
    })
  }
  ...
  pay_button.onclick = async e => {
    ...
    if (!pay_el) {
      pay_el = await pay_invoice(subs[2], {
        ...
        onDecode: (invoice) => sendUp({ from: 'lightning_buttons', type: 'decode_ln_invoice', data: { invoice } }),
        onPay: (invoice) => sendUp({ from: 'lightning_buttons', type: 'pay_ln_invoice', data: { invoice } })
      })
      pay_overlay.appendChild(pay_el)
    }
    // Reset to fresh state every time the panel opens
    if (typeof pay_el.reset === 'function') pay_el.reset()
  ...
  create_button.onclick = async e => {
    ...
    if (!create_el) {
      create_el = await create_invoice(subs[3], {
        ...
        onCreate: (data) => sendUp({ from: 'lightning_buttons', type: 'create_ln_invoice', data })
      })
      create_overlay.appendChild(create_el)
      // Push any already cached templates 
      if (currentTemplates.length > 0 && typeof create_el.updateTemplates === 'function') {
        create_el.updateTemplates(currentTemplates)
      }
      if (typeof create_el.updateContacts === 'function') {
        create_el.updateContacts(currentContacts)
      }
    }
    if (typeof create_el.reset === 'function') create_el.reset()
    if (typeof create_el.updateBalance === 'function') {
      create_el.updateBalance(currentLnBalance)
    }
