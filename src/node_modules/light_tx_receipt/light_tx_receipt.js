  ...
  el._update = function (rows) {
    ...
    rows.forEach(function (r) {
      ...
      const convertHtml = r.convert
        ? '<div class="convert-buttons"><button class="convert-btn sats active">SATS</button><button class="convert-btn usd">USD</button></div>'
        : ''
      s.innerHTML = `
      ...
      `
      // Wire up SATS/USD conversion buttons
      if (r.convert) {
        const satsBtn = s.querySelector('.convert-btn.sats')
        const usdBtn = s.querySelector('.convert-btn.usd')
        const valueEl = s.querySelector('.receipt-value span')
        const originalValue = r.value || ''
        if (satsBtn && usdBtn && valueEl) {
          satsBtn.addEventListener('click', function () {
            satsBtn.classList.add('active')
            usdBtn.classList.remove('active')
            valueEl.textContent = originalValue
          })
          usdBtn.addEventListener('click', async function () {
            usdBtn.classList.add('active')
            satsBtn.classList.remove('active')
            // Extract numeric sats from the original value (e.g. "25 sats" -> 25)
            const satsMatch = originalValue.match(/([\d.]+)/)
            if (satsMatch) {
              const sats = parseFloat(satsMatch[1])
              const btcAmount = sats / 100000000
              try {
                const rate = await btc_usd_rate('btc', 'usd')
                if (rate) {
                  const usd = (btcAmount * rate).toFixed(2)
