...
async function home_contents(opts = {}) {
  ...
  // -------------------- net_helper setup --------------------
  const { io: net_io, _ } = net(id)
  net_io.on.backend = io_backend()
  // Wire the WebSocket tunnel — this populates _.backend and binds incoming rx
  net_io.connect('backend', ws_client)
  ...
  function io_backend() {
    ...
    function on__response(m) {
      // Backend echoes the original mid back in head[2]
      const mid = m.head?.[2]
