...
async function home_contents(opts = {}) {
  ...
  function fetchNodeInfo() {
    const head = io.send('lightning-getinfo', { nodeId: 'node4' })
  ...
  function fetchBalances() {
    const btcHead = io.send('get_btc_balance')
    wait[btcHead[2]] = function (m) {
      currentBtc = m.data?.data?.btc || 0
      updateUI()
    }
    const lnHead = io.send('get_ln_balance', { userId: 'node4' })
  ...
  function fetchBtcTransactions() {
    const head = io.send('list_btc_transactions', { count: 20, skip: 0 })
    wait[head[2]] = function (m) {
  ...
  function fetchLnTransactions() {
    const head = io.send('get_transaction_history', { userId: 'node4', limit: 20 })
    wait[head[2]] = function (m) {
  ...
  io.on.home_contents = io_home_contents()
