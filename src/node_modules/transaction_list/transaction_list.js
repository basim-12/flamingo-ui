...
async function transaction_list(opts = {}, protocol) {
  ...
  function io_backend() {
    return protocol
    function protocol(m) {
      const handler = wait[m.refs?.cause] //wait[mid] removed
      if (!handler) return
      delete wait[m.refs.cause]
      handler(m)
    }
  }
  ...
  async function showDetail(tx, subs) {
    ...
    wait[head] = function (m) { // wait[head[2]] removed
      if (receiptComponent && receiptComponent._update) {
