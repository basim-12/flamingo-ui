...
async function transaction_list(opts = {}, protocol) {
  ...
  function buildRows(txData, tx) {
    if (txData) {
      ...
      // Separate entries by direction — orphan/generate/immature/receive are all incoming
      ...
      // Determine direction from the tx list data (tamount sign) or from details
      const isSend = tx.tamount ? tx.tamount.startsWith('-') : sends.length > 0 && inbound.length === 0

      if (isSend && sends.length > 0) {
        // Outgoing: sum up all send amounts
        ...
        return [
          ...
          { label: 'Total Amount', value: (totalSent + fee).toFixed(8) + ' BTC', icon: 'btc.svg', convert: true }
        ]
      } else if (inbound.length > 0) {
        // Incoming: sum up all inbound amounts (receive, generate, orphan, immature)
        ...
        return [
          ...
          { label: 'Status', value: status },
          { label: 'Blockchain Explorer', value: 'https://mempool.space/tx/' + txid, link: true },
          { label: 'Total Amount', value: totalReceived.toFixed(8) + ' BTC', icon: 'btc.svg', convert: true }
        ]
      }

      // Fallback: use details amounts if available, otherwise top-level amount
