...
async function light_tx_list(opts = {}) {
  ...
  function buildRows(invoice, tx) {
    if (invoice) {
      const paidAt = invoice.paid_at ? new Date(invoice.paid_at * 1000) : null
      const timeStr = paidAt
        ? paidAt.toLocaleDateString('en-US', { day: 'numeric', month: 'long', year: 'numeric' }) + ', ' + paidAt.toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' })
        : '-'
      const amountMsat = invoice.amount_received_msat || invoice.amount_msat || 0
      const amountSats = Math.round(amountMsat / 1000)
      const isSend = tx.tamount && tx.tamount.startsWith('-')
      return [
        { label: isSend ? 'Paid To' : 'Received From', value: tx.tid || '-' },
        { label: 'Description', value: invoice.description || invoice.label || '-' },
        { label: 'Time & Date', value: timeStr },
        { label: 'Amount (sats)', value: amountSats + ' sats', icon: 'lightning.svg', convert: true },
        { label: 'Status', value: invoice.status || '-' },
        { label: 'Payment Hash', value: invoice.payment_hash || '-' },
      ]
    }
    ...
    return [
      ...
      { label: 'Amount', value: (tx.tamount || '-') + ' BTC', icon: 'lightning.svg', convert: true },
    ]
  }
