...
async function wallet_button(opts = {}, protocol) {
  ...
  const senders = { wallet_general: null }
  ...
  const { io, _: net_send } = net(id)
  io.on.home_contents = function (m) {
    if (m.type !== 'update') return
    latestBalances = m.data
    if (switch_el) net_send.switch_account('update', { cause: [] }, m.data)
  }
  io.on.switch_account = function () {}
  el.invite = io.invite('home_contents', { wallet_button: id })
  ...
  wallet_btn.onclick = async (event) => {
    ...
    if (!switch_el) {
      const switchProtocol = (msg) => {
        net_send.home_contents('switch', { cause: [] }, msg)
      }
      switch_el = await switch_account(subs[1], switchProtocol)
      io.accept(switch_el.invite)
      net_send.switch_account('update', { cause: [] }, latestBalances)
